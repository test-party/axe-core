...
/**
 * Get the full list up to the top for TestParty id field.
 */
function getElmTestPartyIdList(elm) {
  var list = '';
  do {
    const attribute = elm.getAttribute('testparty-id');
    if (attribute) {
      if (list.length <= 0) {
        list = attribute;
      } else {
    ...
    elm &&
    elm.nodeType !== 11 &&
    !(elm.nodeType === 1 && elm.nodeName === 'BODY')
  );
  return list;
}
...
function generateSelector(elm) {
  ...
  const selector = getElmTestPartyIdList(elm);
  return selector;
 ...
 */
export default function getSelector(elm) {
  return getShadowSelector(generateSelector, elm);
}

...
function setTestpartyId(node, uuid) {
  node.setAttribute('testparty-id', uuid);
}
...
describe('axe.utils.getSelector', function () {
  ...
  it('should generate a unique CSS selector', function () {
    var node = document.createElement('div');
    fixtureSetup(node);
    let id = axe.utils.uuid();
    setTestpartyId(node, id);
    var sel = axe.utils.getSelector(node);

    assert.equal(sel, `123456 > ${id}`);
  });
  ...
  it('should be able to fall back to positional selectors', function () {
    ...
    fixtureSetup(nodes);
    var sel = axe.utils.getSelector(expected);
    assert.equal(sel, `123456 > ${expectedId}`);
  });
